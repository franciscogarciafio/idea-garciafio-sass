...
const sumaTotal = (precio) => {
    let total = (carritoDeCompras.reduce((acumulador, elemento) => acumulador + elemento.precio * elemento.cantidad, 
0)).toFixed(2);     
    total >=1 ? precio.innerHTML = `<h5 class= "precioSumaTotal">El precio total es $ ${total}</h5>` : precio.
innerHTML = `<h5>Aún no hay productos en el carrito</h5>`;
}
...
const contador = () => {
    ...
    contadorCarrito.textContent = carritoDeCompras.reduce((acumulador, elemento) => acumulador + elemento.cantidad, 
0);
}
...
const cerrarLogin = () => {
  if(sessionStorage.length == 0 || usuarioStorage == ``){
    ...
    Swal.fire({ztitle: '¿Desea cerrar sesión?',
      icon: 'warning',
      showCancelButton: true,
